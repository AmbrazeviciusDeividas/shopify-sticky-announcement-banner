...
class EnsureBilling
{
     ...
     */
    public static function check(Session $session, string $plan): array
    {
        $config = Config::get("shopify.plans.{$plan}");

        if ($config['required'] === false) {
            return [true, null]; // No billing required for the free plan
        }
    ...
    {
        return in_array($config["interval"], self::$RECURRING_INTERVALS, true);
    }
    ...
    private function isUserOnPlan($user, $plan)
    {
    ...
    {
        if (!$this->isUserOnPlan($request->user(), $plan)) {
            // Redirect the user to the billing page for the specified plan
            return redirect()->route('billing', ['plan' => $plan]);
        }
